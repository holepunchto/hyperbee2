...
for await (const data of b1.createDiffStream(b2)) {
    console.log(
        data.left?.key.toString(), 
        data.right?.key.toString(),
    );
}
```
...
Either updates an existing entry with `key` to point to `value` or
inserts a new entry.

#### tryDelete(key)

Removes the entry with `key`, if it exists. If it does not exist, this
method does nothing (and will not throw).
