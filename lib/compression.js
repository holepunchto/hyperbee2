...
class CompressedArray {
  ...
  reset() {
    if (this.updates > 0 || this.uentries !== null) {
      this.commit()
    }
  }
  ...
  flush(max, min) {
    // the raw entries are smaller than the delta, rebase to a single cohort
    if (this.entries.length < this.delta.length) {
      const co = new DeltaCohort(true, null, [])
      for (let i = 0; i < this.entries.length; i++) {
        co.deltas.push(new DeltaOp(true, OP_INSERT, i, this.entries[i]))
      }
      this.delta = [co]
      return this.delta
    }
    ...
    const l = direct.length

    if (l > min && l < this.entries.length && !overflow && this.delta.length + l <= max) {
      const co = new DeltaCohort(true, null, [])
