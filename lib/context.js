...
class CoreContext {
  constructor(store, local, opts = {}) {
    ...
    this.maxKeys = 2 * this.t - 1
    this.getEncryption = opts.getEncryption
    this.lock = opts.lock || new ScopeLock()
    this.other = opts.other || new Map()
    this.length = 0
    this.checkpoint = 0
    this.opened = []
    this.cores = []
  ...
  getCore(index) {
    ...
    if (this.opened[index - 1] === null) {
      ...
      const encryption = this.getEncryption(key)
      this.opened[index - 1] = this.store.get({ key, encryption })
    }
  ...
  getContextByKey(key) {
    ...
    const encryption = this.getEncryption(key)
    const ctx = this._createContext(this.store.get({ key, encryption }))
  ...
  _createContext(core) {
    ...
    const getEncryption = this.getEncryption
    const t = this.t
    const lock = this.lock
    const other = this.other
    return new CoreContext(store, local, { cache, core, getEncryption, t, lock, other })
  }
}
