...
const { decodeBlock, TYPE_LATEST, TYPE_COMPAT } = require('./encoding.js')
...
class CoreContext {
  ...
  async getBlock(seq, core, config) {
    ...
    const block = decodeBlock(buffer, seq)
    // If a degree was loaded (persistedDegree !== 0), skip as it must be more recent.
    if (block.type === TYPE_COMPAT && this.t !== DEGREE_COMPAT && this.persistedDegree === 0) {
