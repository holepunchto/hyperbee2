...
const { decodeBlock, peekBlockType, TYPE_LATEST, TYPE_COMPAT } = require('./encoding.js')
...
class CoreContext {
  constructor(
    ...
    this.cores = []
    this.changed = false
    this.persistedDegree = null // never synced with a checkpoint yet
  ...
  async _inflateCheckpoint(length, block, config) {
    ...
    if (checkpoint > 0) {
      ...
      if (block.metadata && block.metadata.degree) {
        const degree = block.metadata.degree

        // Don't let an up-to-date checkpoint clobber a pending degree
        // change that hasn't been flushed yet (setDegree(), compat auto-detect)
        // Only sync when we've never synced before (persistedDegree === null), or our current t
        // already matches what we last confirmed was on disk.
        const canSync = this.persistedDegree === null || this.t === this.persistedDegree
        if (degree && canSync) {
          this._setDegree(degree)

          // merge, don't overwrite
          this.persistedDegree = degree || this.persistedDegree
        }
      }
    }
  ...
  async getBlock(seq, core, config) {
    ...
    // decodeBlock() converts legacy blocks into modern shape so we peek instead
    // to get the type to check compat type. If a degree was loaded
    // (persistedDegree !== null), skip as it must be more recent.
    if (peekBlockType(buffer) === TYPE_COMPAT && this.t !== DEGREE_COMPAT && this.persistedDegree === null) {
      this._setDegree(DEGREE_COMPAT)
    }
