...
class CoreContext {
  constructor(
    ...
    this.cores = []
    this.changed = false
    this.persistedDegree = 0 // never synced with a checkpoint yet
  ...
  async _inflateCheckpoint(length, block, config) {
    ...
    if (checkpoint > 0) {
      ...
      if (block.metadata && block.metadata.degree) {
        ...
        // Only sync when we've never synced before (persistedDegree === 0), or our current t
        // already matches what we last confirmed was on disk.
        const canSync = this.persistedDegree === 0 || this.t === this.persistedDegree
        if (degree && canSync) {
  ...
  async getBlock(seq, core, config) {
    ...
    // (persistedDegree !== 0), skip as it must be more recent.
    if (
      peekBlockType(buffer) === TYPE_COMPAT &&
      this.t !== DEGREE_COMPAT &&
      this.persistedDegree === 0
    ) {
