...
class CoreContext {
  constructor(store, local, core, encryption, other = new Map()) {
    this.store = store
    this.local = local
    this.core = core
  ...
  getContextByKey(key) {
    ...
    const core = this.store.get({ key, encryption: this.encryption })
    const ctx = new CoreContext(this.store, this.local, core, this.encryption, this.other)
  ...
  async getContext(core, activeRequests) {
    ...
    const hc = this.getCore(core)
    const ctx = new CoreContext(this.store, this.local, hc, this.encryption, this.other)
    this.other.set(hex, ctx)
    return ctx
  }
}
