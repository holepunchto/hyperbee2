...
module.exports = async function copyChanges(tree, changes, config) {
  ...
  const localHex = b4a.toString(local.core.key, 'hex')

  const oldest = changes[changes.length - 1]
  const tip = local.core.length
  ...
  for (let i = changes.length - 1; i >= 0; i--) {
    ...
    if (change === oldest) {
      last.previous = await previousForOldest(src, change.tail, tip)
    } else {
      last.previous = { core: 0, seq: start - 1 }
    }

    if (local.changed) {
      local.checkpoint = start + batch.length
      last.metadata = local.flush()
    }

    const buffers = new Array(batch.length)

    for (let j = 0; j < batch.length; j++) {
      batch[j].checkpoint = local.checkpoint
      buffers[j] = encodeBlock(batch[j])
    }
  ...
  async function previousForOldest(src, tail, tip) {
    if (tail === null) return null

    const hex = b4a.toString(tail.key, 'hex')
    if (hex === localHex) return { core: 0, seq: tail.length - 1 }

    if (tip > 0) return { core: 0, seq: tip - 1 }
