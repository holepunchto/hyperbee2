  ...
  async _next() {
    ...
    while (true) {
      ...
      if (l.isKey && r.isKey) {
        ...
        if (c === 0) {
          const [left, right] = await Promise.all([a.nextKey(), b.nextKey()])
...
class TreeCursor {
  ...
  async open() {
    ...
    this._stack.push({ node: this.root, offset: 0, value: null })
  ...
  async _inflate(entry) {
    if (entry.value) return this.tree.bump(entry.node)
  ...
  skip() {
    ...
    if (top.offset >= maxOffset) this._stack.pop()
  }
...
function samePosition(a, b) {
  ...
  return b4a.equals(a.coreKey, b.coreKey)
}
