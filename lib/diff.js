...
class DiffIterator {
  constructor(tree, left, right, opts = {}) {
    const { limit = -1, config = left.config } = opts
...
class DiffStream extends Readable {
  ...
  _predestroy() {
    Hypercore.destroyRequests(this.iterator.left.config.activeRequests, null)
    Hypercore.destroyRequests(this.iterator.right.config.activeRequests, null)
  }
}
