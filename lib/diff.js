...
class DiffIterator {
  ...
  _fastForward() {
    while (this.left.stack.length && this.right.stack.length) {
      ...
      if (a.node.seq !== b.node.seq) return
      if (a.node.core !== b.node.core) return
      if (a.node.context.id !== b.node.context.id) return
...
function isSame(a, b) {
  return a.seq === b.seq && a.core === b.core && a.context.core.id === b.context.core.id
}
