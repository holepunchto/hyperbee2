...
exports.peekBlockType = peekBlockType
...
// The on-disk type discriminator, read without fully decoding the block.
// Use this whenever you need to know what was actually written to disk,
// e.g. to detect legacy/compat data.
function peekBlockType(buffer) {
...
function decodeBlock(buffer, seq) {
  const type = peekBlockType(buffer)
  const state = { start: 0, end: buffer.byteLength, buffer }
...
function decodeBlock0(state, seq) {
  ...
  return {
    ...
    metadata: { cores: blk.cores, degree: 0 },
    tree,
