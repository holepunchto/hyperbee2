...
exports.inflate = async function inflate(ptr, config) {
  ...
  // Dedupe concurrent inflates of the same node on the same session (activeRequests)
  // Callers on a different session must never await request from another session
  const session = config.activeRequests

  if (!ptr.inflating) ptr.inflating = new Map()

  let inflating = ptr.inflating.get(session)
  if (!inflating) {
    inflating = _inflate(ptr, config)
    ptr.inflating.set(session, inflating)
  }

  try {
    return await inflating
  } finally {
    // Dedupe deleting from multiple callers on the same session.
    if (ptr.inflating && ptr.inflating.get(session) === inflating) {
      ptr.inflating.delete(session)
      if (ptr.inflating.size === 0) ptr.inflating = null
    }
  }
}
...
async function _inflate(ptr, config) {
  ...
  const value = new TreeNode(block.t, k, c)
