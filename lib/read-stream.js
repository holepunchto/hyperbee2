    ...
    while (true) {
      ...
      for (let i = 0; i < top.node.value.keys.length; i++) {
        const j = this._reverse
          ? top.node.value.keys.length - 1 - i
          : i

        const c = this._reverse
          ? b4a.compare(top.node.value.keys[j].key, this._end)
          : b4a.compare(this._start, top.node.value.keys[j].key)
      ...
      const j = this._reverse
        ? top.node.value.children.length - 1 - k
        : k

      this._stack.push({
        node: top.node.value.children[j],
        offset: 0
      })
  ...
  async _readp () {
    while (this._stack.length && (this._limit === -1 || this._limit > 0)) {
      ...
      if (child) {
        ...
        if (k < top.node.value.children.length) {
          const j = this._reverse
            ? top.node.value.children.length - 1 - k
            : k
          this._stack.push({ node: top.node.value.children[j], offset: 0 })
        }
        continue
      }
      ...
      if (k < top.node.value.keys.length) {
        const j = this._reverse
          ? top.node.value.keys.length - 1 - k
          : k
