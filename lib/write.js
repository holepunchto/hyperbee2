...
module.exports = class WriteBatch {
  ...
  async flush() {
    ...
    const context = this.key
      ? this.tree.context.getContextByKey(this.key)
      : this.tree.context
  ...
  async _put(key, value) {
    ...
    while (true) {
      const v = ptr.value
        ? this.snapshot.bump(ptr)
        : await this.snapshot.inflate(ptr)
    ...
    while (needsSplit) {
      ...
      if (parent) {
        const p = parent.value
          ? this.snapshot.bump(parent)
          : await this.snapshot.inflate(parent)
  ...
  async _setKeyToNearestLeaf(v, index, stack) {
    ...
    if (ls < rs) {
      ...
      stack.push(right)
      let r = right.value
        ? this.snapshot.bump(right)
        : await this.snapshot.inflate(right)
      ...
      stack.push(left)
      let l = left.value
        ? this.snapshot.bump(left)
        : await this.snapshot.inflate(left)
  ...
  async _rebalance(stack) {
    ...
    const r = root.value
      ? this.snapshot.bump(root)
      : await this.snapshot.inflate(root)
  ...
  async _flush() {
    ...
    if (blocks.length > 0 && this.length > 0) {
      const core = this.key
        ? await this.tree.context.getCoreOffsetByKey(
            this.key,
            this.tree.activeRequests
          )
        : 0
