...
module.exports = class WriteBatch {
  ...
  async flush() {
    ...
    try {
      ...
      const localContext = this.tree.context.getLocalContext()
      await localContext.update(this.config)
      ...
      await this._flush(localContext)
      await this.close()
  ...
  async _flush(context) {
    ...
    if (this.root.context !== context) {
      if (!this.root.context.metadataLoaded) await this.root.context.update(this.config)
      if (this.root.context.t !== context.t) {
        context._setDegree(this.root.context.t)
        context.changed = true // Degree change requires a checkpoint
      }
    }
    ...
    if (blocks.length > 0 && this.length > 0) {
      const core = this.key ? await context.getCoreOffsetByKey(this.key, this.config) : 0
