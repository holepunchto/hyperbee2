...
module.exports = class WriteBatch {
  ...
  async _flush() {
    ...
    while (stack.length > 0) {
      ...
      node.value.keys.flush(this.deltaMax, this.deltaMin)
      node.value.children.flush(this.deltaMax, this.deltaMin)
      ...
      const keys = node.value.keys.entries
      const children = node.value.children.entries
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      for (const n of update.nodes) {
        ...
        const treeDelta = {
          keys: n.value.keys.delta,
          children: n.value.children.delta
        }
console.log('treeDelta', treeDelta)
...
function getEstimatedNodeSize(n) {
  return n.keys.delta.length * ESTIMATED_POINTER_SIZE + n.children.delta.length * ESTIMATED_POINTER_SIZE
}
