...
module.exports = class WriteBatch {
  ...
  async _rebalance (stack) {
    ...
    while (stack.length > 1) {
      ...
      let r = right && (right.value ? this.snapshot.bump(right) : await this.snapshot.inflate(right))
  ...
  async _flush () {
    ...
    while (stack.length > 0) {
      ...
      for (let i = 0; i < node.value.keys.length; i++) {
        ...
        if (!k.changed) {
          k.core = await this.tree.context.getCoreOffset(k.context, k.core, this.tree.activeRequests)
      ...
      for (let i = 0; i < node.value.children.length; i++) {
        ...
        if (!n.changed) {
          n.core = await this.tree.context.getCoreOffset(n.context, n.core, this.tree.activeRequests)
    ...
    if (blocks.length > 0 && this.length > 0) {
      const core = this.key ? await this.tree.context.getCoreOffsetByKey(this.key, this.tree.activeRequests) : 0
      blocks[blocks.length - 1].previous = { core, length: this.length }
    }
