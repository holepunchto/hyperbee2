  ...
  async _flush() {
    ...
    while (stack.length > 0) {
      ...
      for (let i = 0; i < node.value.keys.length; i++) {
        ...
        if (ptr) {
// console.log('has ptr', ptr.core, ptr.context.core.id, context.core.id)
//           ptr.core = await context.getCoreOffset(ptr.context, ptr.core, activeRequests)
//           ptr.context = context
        }

        if (!k.changed) {
          k.core = await context.getCoreOffset(k.context, k.core, activeRequests)
          k.context = context
          continue
        }
      ...
      for (let i = 0; i < node.value.children.length; i++) {
        ...
        if (!n.changed) {
          n.core = await context.getCoreOffset(n.context, n.core, activeRequests)
          n.context = context
          continue
        }
