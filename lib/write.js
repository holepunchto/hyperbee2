...
module.exports = class WriteBatch {
  ...
  async _put(key, value) {
    ...
    while (true) {
      const v = ptr.value || (await snap.inflate(ptr))
    ...
    while (status === NEEDS_SPLIT) {
      ...
      if (parent) {
        const p = parent.value || (await snap.inflate(parent))
  ...
  async _delete(key) {
    ...
    while (true) {
      const v = ptr.value || (await this.snapshot.inflate(ptr))
      stack.push(ptr)
  ...
  async _setKeyToNearestLeaf(v, index, stack) {
    ...
    if (ls < rs) {
      ...
      stack.push(right)
      let r = right.value || (await snap.inflate(right))
      while (r.children.ulength) {
        right = r.children.uget(0)
        stack.push(right)
        r = right.value || (await snap.inflate(right))
      }
      v.keys.set(index, r.keys.shift())
    } else {
      // if fewer leaves on the right
      stack.push(left)
      let l = left.value || (await snap.inflate(left))
      while (l.children.ulength) {
        left = l.children.uget(l.children.length - 1)
        stack.push(left)
        l = left.value || (await snap.inflate(left))
      }
  ...
  async _leafSize(ptr, goLeft) {
    ...
    let v = ptr.value || (await snap.inflate(ptr))
    while (v.children.ulength) {
      ptr = v.children.uget(goLeft ? 0 : v.children.length - 1)
      v = ptr.value || (await snap.inflate(ptr))
    }
  ...
  async _rebalance(stack) {
    ...
    while (stack.length > 1) {
      ...
      let l = left && (left.value || (await snap.inflate(left)))
      ...
      let r = right && (right.value || (await snap.inflate(right)))
    ...
    const r = root.value || (await snap.inflate(root))
