...
module.exports = class WriteBatch {
  ...
  async _flush() {
    ...
    const supportsCompression = this.type !== TYPE_COMPAT && this.type > 0
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      for (const n of update.nodes) {
        ...
        const max = supportsCompression ? this.deltaMax : 0
        const min = supportsCompression ? this.deltaMin : Infinity

        const treeDelta = {
          keys: n.value.keys.flush(max, min),
          children: n.value.children.flush(max, min)
        }
