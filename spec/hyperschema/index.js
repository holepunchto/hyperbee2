...
const encoding13 = {
  preencode(state, m) {
    ...
    if (version >= 2 && m.degree) c.uint.preencode(state, m.degree)
  },
  encode(state, m) {
    const flags = version >= 2 && m.degree ? 1 : 0

    encoding13_0.encode(state, m.cores)
    c.uint.encode(state, flags)

    if (version >= 2 && m.degree) c.uint.encode(state, m.degree)
  },
  decode(state) {
    ...
    return {
      cores: r0,
      degree: version >= 2 && (flags & 1) !== 0 ? c.uint.decode(state) : 0
    }
  }
}
